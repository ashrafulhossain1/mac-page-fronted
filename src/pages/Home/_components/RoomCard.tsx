...
const RoomCard = ({
  ...
  return (
    <Link to={`/browse/${id}`} className="block">
      <Card className="w-full md:max-w-104 overflow-hidden rounded-[32px] p-4 group hover:shadow-xl transition-all duration-100 border">
        {/* Image Section */}
        <CardHeader className="p-0 relative overflow-hidden  rounded-[16px]">
          <img
            src={image}
            alt={title}
            className="h-67.5 w-full object-cover transition-transform duration-500 group-hover:scale-103 rounded-[16px]"
          />
