...
export default function Reviews() {
  ...
  // Maximum index we can scroll to before showing empty space
  ...
  return (
    <section className="w-full mt-16 mb-8 overflow-hidden">
      <div className="w-full">
        {/* Title */}
        <h2 className="text-3xl md:text-4xl font-bold mb-10 text-primary-foreground">Reviews</h2>

        {/* Carousel Viewport */}
        <div className="relative">
          <motion.div
            animate={{ x: `calc(-${index * (100 / perView)}% - ${index * (32 / perView)}px)` }}
          ...
          >
            {reviewsData.map((review) => (
              <div
                key={review.id}
                className="min-w-[calc((100%-64px)/3)] w-[calc((100%-64px)/3)] flex-shrink-0"
              >
                <ReviewCard
        ...
        <div className="flex justify-center items-center gap-4 mt-12">
          <button
            onClick={prev}
            disabled={index === 0}
            className={`flex items-center justify-center w-14 h-14 rounded-[20px] transition-all duration-300 ${index === 0
          ...
          <button
            ...
            className={`flex items-center justify-center w-14 h-14 rounded-[20px] transition-all duration-300 ${index >= maxIndex
              ? "bg-gray-100 text-gray-300 cursor-not-allowed"
              : "bg-gray-100 text-gray-500 hover:bg-gray-200 cursor-pointer shadow-sm hover:shadow-md"
              }`}
          >
